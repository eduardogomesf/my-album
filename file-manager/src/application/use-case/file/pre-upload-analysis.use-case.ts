...
export class PreUploadAnalysisUseCase implements UseCase {
  ...
  async execute(
    params: PreUploadAnalysisUseCaseParams,
  ): Promise<UseCaseResponse<PreUploadAnalysisUseCaseResponse>> {
    ...
    const { hasFreeSpace, freeSpace } = await this.canAddMoreFiles(
      params.userId,
    )
    ...
    const filesAfterAnalysis = await this.analyzeFiles(
      orderedFiles,
      freeSpace,
      params.userId,
      params.albumId,
  ...
  private async analyzeFiles(
    ...
    for await (const file of files) {
      ...
      const uploadUrlDetails =
        await this.generateUploadUrlService.generateUploadUrl({
          id: newFile.id,
  ...
  private async canAddMoreFiles(
    userId: string,
  ): Promise<{ freeSpace: number; hasFreeSpace: boolean }> {
    const { usage } =
      await this.getCurrentStorageUsageRepository.getUsage(userId)

    const maxStorageSize = megaBytesToBytes(
