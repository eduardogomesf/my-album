...
export class GetFilesByAlbumIdUseCase implements UseCase {
  ...
  async execute(
    params: GetFilesByAlbumIdUseCaseParams,
  ): Promise<UseCaseResponse<GetFilesByAlbumIdUseCaseResponse>> {
    const albumById = await this.getAlbumByIdRepository.getById(
      params.albumId,
      params.userId,
    )
    ...
    const filters = this.handleFilters(
      params.filters,
      albumById.status === AlbumStatus.ACTIVE,
    )
    ...
    const filesWithUrls: GetFilesByAlbumIdFile[] = await Promise.all(
  ...
  private handleFilters(
    rawFilters: GetFilesByAlbumIdFilters,
    isAlbumActive = true,
  ) {
    return {
