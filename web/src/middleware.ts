...
async function refreshToken(request: NextRequest, userId: string, refreshToken: string) {
...
export async function middleware(request: NextRequest) {
  ...
  if(!accessTokenCookie?.value) {
    ...
    if(!refreshTokenCookie?.value || !userIdCookie?.value) {
      ...
      return await refreshToken(request, userIdCookie.value, refreshTokenCookie.value)
    }
  }
